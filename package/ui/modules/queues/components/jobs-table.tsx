...
export function JobsTable({ jobs, queueName, onJobsChange }: JobsTableProps) {
  ...
  const handleDeleteJob = async (queueName: string, jobId: string) => {
    ...
    try {
      setIsDeleting(true);
      await queuesApi.deleteJob(queueName, jobId);
      toast.toast({ title: 'Success', description: 'Job deleted successfully' });
      onJobsChange();
    } catch (error) {
      toast.toast({
        title: 'Error',
  ...
  const handleDeleteAllJobs = async (queueName: string) => {
    if (!window.confirm('Are you sure you want to delete all non-active jobs in this queue?')) return;
    try {
      setIsDeletingAll(true);
      await queuesApi.deleteAllJobs(queueName);
      toast.toast({ title: 'Success', description: 'All jobs deleted successfully' });
      onJobsChange();
    } catch (error) {
      toast.toast({
        title: 'Error',
